    ...
    reader.onload = (evt) => {
      try {
        ...
        const subjectHeader = headers.find(
          (h) => normalizeHeader(h) === "subjectline",
        );
        const bodyHeader = headers.find(
          (h) => normalizeHeader(h) === "fullemailbody",
        );
        const companyHeader = headers.find(
          (h) => normalizeHeader(h) === "company",
        );

        if (!emailHeader || !subjectHeader || !bodyHeader) {
          throw new Error(
            `Missing required columns! The file must exactly have: "Email", "Subject Line", and "Full Email Body". (Found: ${headers.join(", ")})`,
          );
        }
        ...
        data.forEach((row) => {
          ...
          if (
            !subjectLine ||
            !fullEmailBody ||
            !subjectLine.toString().trim() ||
            !fullEmailBody.toString().trim()
          ) {
